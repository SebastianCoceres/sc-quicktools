// Regex for RGB and RGBA
const rgbRgbaRegex = /rgba?\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})(?:\s*,\s*(0|1|0?\.\d+))*\s*\)/i;

// Regex for HSL and HSLA
const hslHslaRegex = /hsla?\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})%\s*,\s*([0-9]{1,3})%(?:\s*,\s*(0|1|0?\.\d+))?\s*\)/i;

// Regex for Hexadecimal
const hexRegex = /#([a-fA-F0-9]{3,4}|[a-fA-F0-9]{6,8})\b/i;

// Regex for Named Colors
const namedColorRegex = /\b(black|white|gray|grey|red|green|blue|yellow|cyan|magenta|orange|purple|pink|brown)\b/;
...
export function DetectColorFormat(color: string) {
    ...
    if (color.match(rgbRgbaRegex)) {
        return "rgba"
    }
    if (color.match(hslHslaRegex)) {
        return "hsla"
    }
    if (color.match(namedColorRegex)) {
        return "named"
    }
...
export function rgbaToHex(color: string) {
    const rgba = color.match(rgbRgbaRegex)?.slice(1, 4) ?? ["0", "0", "0", "1"]

    return `#${rgbToHex(rgba[0], rgba[1], rgba[2])}${Math.round(parseFloat(rgba[3]) * 255).toString(16).padStart(2, "0")}`
}
...
export function hslaToHex(color: string) {
    const hsla = color.match(hslHslaRegex)?.slice(1, 4) ?? ["0", "0", "0", "1"]

    return `#${hslToHex(hsla[0], hsla[1], hsla[2])}${Math.round(parseFloat(hsla[3]) * 255).toString(16).padStart(2, "0")}`
}

export function hslToHex(h: string, s: string, l: string) {
    const _l = parseInt(l)
    const _s = parseInt(s) / 100
    const _h = parseInt(h) / 360

    const c = (1 - Math.abs(2 * _l - 1)) * _s
    const x = c * (1 - Math.abs((_h * 6) % 2 - 1))
    const m = _l - c / 2
    const r = 255 * (c + m)
    const g = 255 * (x + m)
    const b = 255 * m

    return rgbToHex(Math.round(r).toString(16).padStart(2, "0"), Math.round(g).toString(16).padStart(2, "0"), Math.round(b).toString(16).padStart(2, "0"))
}

export function namedColorToHex(color: string) {
    const namedColor = color.match(namedColorRegex)?.[0]
    ...
    return COLORS[namedColor as keyof typeof COLORS]
}
